...
def add_elastic_transform(image, alpha, sigma, pad_size=30, seed=None, img_size=(512, 512)):
    """
    Args:
    ...
    return cropping(map_coordinates(image, indices, order=1).reshape(shape), img_size, pad_size, pad_size), seed
...
def add_gaussian_noise(image, mean=0, std=1):
    ...
    image = noise_img / (1 + gaus_noise.max())
    return image
...
def normalization2(image, max=1, min=0):
    ...
    Return :
        image : numpy array of image with values turned into standard scores
    """
    image_new = (image - np.min(image)) * (max - min) / (np.max(image) - np.min(image))
    return image_new
